...
class App(tk.Frame):
    ...
    def __init__(self, window, controller, video_source=0):
        ...
        self.w.pack(pady=10)
        self.add_student = Button(self, text="Add employee", width=17, command=self.save_data, state="disabled")
            ...
            if self.ret:
                ...
                for (top, right, bottom, left), nam, en, cl in zip(self.face_locations, faces_names, enrollm, c_cls):
                    if en not in self.present and nam != "unknown":
                        ...
                        self.count += 1
                        self.t_student += 1
                        self.var.set(str("Total Employee: " + str(self.t_student)))
                        self.update_idletasks()
                    cv2.rectangle(self.frame, (left, top), (right, bottom), (0, 0, 255), 2)
                    cv2.putText(self.frame, en, (left + 10, bottom - 100), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 0, 255),
                                1)
